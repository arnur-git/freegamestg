...
export default function Dashboard() {
  ...
  return <main className="shell">
    ...
    <section id="deals" className="section-head"><div><p className="eyebrow">CURRENT DROPS</p><h2>Активные раздачи <small>{deals.length}</small></h2></div><div className="filters"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск игры..." /><select value={store} onChange={(event) => setStore(event.target.value)}><option value="all">Все магазины</option><option value="epic">Epic Games</option><option value="steam">Steam</option><option value="playstation">PlayStation</option><option value="gog">GOG</option><option value="xbox">Xbox</option><option value="nintendo">Nintendo</option><option value="itchio">itch.io</option><option value="ubisoft">Ubisoft</option><option value="battlenet">Battle.net</option></select><button className={endingSoon ? "filter active" : "filter"} onClick={() => setEndingSoon(!endingSoon)}>⏰ Скоро заканчиваются</button></div></section>
